...
describe('plugin_system_test', async () => {
  ...
  it('通貨形式', async () => {
    ...
    await cmp('123456789を通貨形式。表示。', '123,456,789')
    await cmp('123456789012を通貨形式。表示。', '123,456,789,012')
  })
  it('通貨形式で小数点以下は無視する #1889', async () => {
    await cmp('123456.7891234を通貨形式。表示。', '123,456.7891234')
    await cmp('3.1415を通貨形式。表示。', '3.1415')
  })
  it('ゼロ埋め', async () => {
  ...
  it('Unicodeサロゲートペアで文字数の数え方 - 文字数/何文字目 #1954', async () => {
    await cmp('S=「💧🔥」;Sの文字数を表示', '2')
    await cmp('「💧🔥」で「🔥」が何文字目かを表示', '2')
    await cmp('「あいうえお💧🔥あいうえお」で「🔥」が何文字目かを表示', '7')
    await cmp('「あいうえお💧🔥かきくけこ」で「かき」が何文字目かを表示', '8')
    await cmp('「いろはにほ💧🔥いろいろで」で「いろいろ」が何文字目かを表示', '8')
    await cmp('「いろはにほ💧🔥いろいろで」で「ねこ」が何文字目かを表示', '0')
  })
  it('Unicodeサロゲートペアで文字数の数え方 - 文字挿入 #1954', async () => {
    await cmp('S=「💧🔥」;Sの1に「A」を文字挿入して表示', 'A💧🔥')
    await cmp('S=「💧🔥」;Sの2に「🍵」を文字挿入して表示', '💧🍵🔥')
    await cmp('S=「A💧BC」;Sの3に「🍵」を文字挿入して表示', 'A💧🍵BC')
    await cmp('S=「A💧CD」;Sの5に「🍵」を文字挿入して表示', 'A💧CD🍵')
    await cmp('S=「A💧CD」;Sの6に「🍵」を文字挿入して表示', 'A💧CD🍵')
  })
  it('Unicodeサロゲートペアで文字数の数え方 - 文字検索 #1954', async () => {
    await cmp('S=「💧🔥🍵」;Sの1から「🔥」を文字検索して表示', '2')
    await cmp('S=「💧🔥🍵」;Sの1から「🍵」を文字検索して表示', '3')
    await cmp('S=「💧🔥🍵」;Sの1から「😊」を文字検索して表示', '0')
    await cmp('S=「ABC💧🔥🍵ABC」;Sの4から「ABC」を文字検索して表示', '7')
  })
  it('Unicodeサロゲートペアで文字数の数え方 - 文字抜出/文字左部分/文字右部分 #1954', async () => {
    await cmp('S=「💧🔥🍵」;Sの2から2を文字抜き出して表示', '🔥🍵')
    await cmp('S=「A💧B🔥C🍵」;Sの3から2を文字抜き出して表示', 'B🔥')
    await cmp('S=「💧🔥🍵」;Sの1から(Sの文字数)を文字抜き出して表示', '💧🔥🍵')
    await cmp('S=「💧🔥🍵」;Sの2文字左部分を表示', '💧🔥')
    await cmp('S=「💧🔥🍵」;Sの2文字右部分を表示', '🔥🍵')
  })
  ...
  it('半角スペースを全角変換すると＀になる #1999', async () => {
    ...
    await cmp('「ABC 123 !@#」を全角変換して表示', 'ＡＢＣ　１２３　！＠＃')
    await cmp('「ＡＢＣ　１２３　！＠＃」を英数記号半角変換して表示', 'ABC 123 !@#')
  })
  ...
  it('『文字抜出』でAにマイナス値を指定したとき #1995', async () => {
    await cmp('MID("1234",-2,2)を表示', '34')
    await cmp('「1234」の-2から2を文字抜き出しを表示', '34')
    await cmp('「1234」の-9から2を文字抜き出しを表示', '12') // 範囲を超えて指定もエラーにならない
    await cmp('「1234」の3から-3を文字抜き出しを表示', '')
    await cmp('「1234567890」の「3」から3文字抜出す。それを表示', '345')
  })
  it('「文字始まる」「文字終わる」の実装 #2081', async () => {
    await cmp('もし「やみなべ」が「やみ」で文字始まるならば、「OK」と表示。', 'OK')
    await cmp('もし「やみなべ」が「なべ」で文字終わるならば、「OK」と表示。', 'OK')
    await cmp('もし「やみなべ」が「しか」で文字始まるならば\n「A」と表示\n違えば\n「B」と表示\nここまで。', 'B')
    await cmp('もし「やみなべ」が「しか」で文字終わるならば\n「A」と表示\n違えば\n「B」と表示\nここまで。', 'B')
  })
  it('「出現」の実装 #2084', async () => {
    // 文字列に
    await cmp('もし「やみなべ」に「やみ」が出現するならば、「OK」と表示。', 'OK')
    await cmp('もし「やみなべ」が「うみ」で出現しなければ、「OK」と表示。', 'OK')
    await cmp('もし「強敵が出現した」で「出現」が出現したならば\n「A」と表示\n違えば\n「B」と表示\nここまで。', 'A')
    await cmp('もし「強敵が出現した」で「スター」が出現したならば\n「A」と表示\n違えば\n「B」と表示\nここまで。', 'B')
    // 配列に
